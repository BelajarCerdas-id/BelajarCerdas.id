                    ...
                    <div class="flex flex-col xl:flex-row gap-6">
                        ...
                        <div class="flex-1 overflow-auto bg-white shadow-lg border rounded-md">
                            ...
                            <div class="p-4 rounded-lg mb-6">
                                <p class="text-lg font-bold opacity-70">List Mata Pelajaran</p>
                            </div>

                            <!-- Grid -->
                            <ul class="grid grid-cols-1 sm:grid-cols-2 gap-4 p-4 border-t">
                                @foreach ($getMapelByKelas as $item)
                                    <a
                                        href="{{ route('soalPembahasanBab.view', [$kelas, $kelas_id, $item->mata_pelajaran, $item->id]) }}">
                                        <li
                                            class="flex items-center justify-between gap-2 hover:bg-gray-200 py-2 rounded cursor-pointer px-4 mb-2">
                                            <span
                                                class="font-bold opacity-70 text-sm">{{ $item->mata_pelajaran }}</span>
                                            <i class="fa-solid fa-chevron-right text-xs"></i>
                                        </li>
                                @endforeach
                                </a>
