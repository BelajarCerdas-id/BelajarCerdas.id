@include('components/sidebar_beranda', [
    ...
    'linkBackButton' => route('soalPembahasanMapel.view', [$kelas, $kelas_id]),
                    ...
                    <div class="flex flex-col xl:flex-row gap-6">
                        ...
                        <div class="flex-1 bg-white shadow-lg border rounded-md">
                            ...
                            <div class="p-4 rounded-lg mb-6">
                                <p class="text-lg font-bold opacity-70">List Bab</p>
                            </div>

                            <!-- Grid -->
                            <ul class="grid grid-cols-1 sm:grid-cols-2 gap-4 p-4 border-t max-h-96 overflow-y-auto">
                                @foreach ($getBabByMapel as $item)
                                    <a
                                        href="{{ route('soalPembahasanAssessment.view', [$kelas, $kelas_id, $mata_pelajaran, $mapel_id, $item->id]) }}">
                                        <li
                                            class="flex items-center justify-between gap-2 hover:bg-gray-200 py-2 rounded cursor-pointer px-4 mb-2">
                                            <span class="font-bold opacity-70 text-sm">{{ $item->nama_bab }}</span>
                                            <i class="fa-solid fa-chevron-right text-xs"></i>
                                        </li>
                                    </a>
                                @endforeach
