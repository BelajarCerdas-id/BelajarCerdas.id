...
class EnglishZoneController extends Controller
{
    ...
    public function quizListeningExamTestForm($levelId)
    {
        ...
        $passages = EnglishZonePassage::where('level_id', $levelId)->where('passage_type', 'Listening Exam Test')
        ->where('passage_status', 'Publish')->paginate(1);

        $passageIds = EnglishZonePassage::where('level_id', $levelId)->where('passage_type', 'Listening Exam Test')->pluck('id');
        ...
        // Buat key cache unik berdasar kan setiap hari, user, levelId, subscriptionId, passageId, dan publishedQuestionIds
        $cacheKey = "english-zone-quiz-listening-exam-test-{$userId}-{$levelId}-{$subscriptionId}-{$passageIds}-{$publishedQuestionIds}";
