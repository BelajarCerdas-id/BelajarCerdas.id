...
class EnglishZoneController extends Controller
{
    ...
    public function quizDetailFetch($levelId)
    {
        ...
        return response()->json([
            'data' => $levelName,
            'readingPracticeTest' => '/english-zone/:levelId/quiz/reading-practice-test',
            'readingExamTest' => '/english-zone/:levelId/quiz/reading-exam-test',
            ...
            'speakingPracticeTest' => '/english-zone/:levelId/quiz/speaking-practice-test',
            'speakingExamTest' => '/english-zone/:levelId/quiz/speaking-exam-test',
        ]);
    }

    // function quiz reading practice test view
    public function quizReadingPracticeTest($levelId)
    {
        $user = Auth::user();
        ...
        return view('Features.english-zone.student.quiz.english-zone-quiz-reading-practice-test', compact('levelId'));
    }

    // function quiz reading practice test form
    public function quizReadingPracticeTestForm($levelId)
    {
        // Ambil tanggal hari ini hanya dalam format 'Y-m-d'
        $today = Carbon::now()->format('Y-m-d');
        ...
        // Ambil semua passage Reading Practice Test
        $passageCollection = EnglishZonePassage::where('level_id', $levelId)->where('passage_type', 'Reading Practice Test')
        ...
        $passageCacheKey = "reading-passages-{$today}-{$userId}-{$subscriptionId}-{$levelId}-{$passageIds}-{$lastUpdated}-{$limit}";
        ...
        $cacheKey = "english-zone-quiz-reading-practice-test-{$today}-{$userId}-{$subscriptionId}-{$levelId}-{$activePassageId}-{$lastUpdated}-{$publishedQuestionIds}";
        ...
        if ($subscription) {
            ...
            $questionsAnswer = EnglishZoneAnswers::whereHas('EnglishZoneQuestions', function ($query) {
                $query->where('tipe_soal', 'QUIZ');
            })->where('student_id', $userId)->where('subscription_history_id', $subscription->id)->whereDate('created_at', $today)
    ...
    // function quiz reading practice test answer
    public function quizReadingPracticeTestAnswer(Request $request, $levelId, $passageId)
    {
        // Ambil tanggal hari ini
        $today = Carbon::now()->format('Y-m-d');
    ...
    // function quiz reading exam test view
    public function quizReadingExamTest($levelId)
    {
        $user = Auth::user();
        ...
        return view('Features.english-zone.student.quiz.english-zone-quiz-reading-exam-test', compact('levelId'));
    }

    // function quiz reading exam test form
    public function quizReadingExamTestForm($levelId)
    {
        // Ambil tanggal hari ini hanya dalam format 'Y-m-d'
        $today = Carbon::now()->format('Y-m-d');
        ...
        // Hitung skor ujian dengan menjumlahkan skor dari soal-soal yang sudah dijawab
        $scoreExam = round($questionsAnswer->sum('question_score'), 2); // untuk menampilkan 2 angka dibelakang koma jika ada
    ...
    public function quizReadingExamTestAnswer(Request $request, $levelId, $passageId)
    {
        // Ambil tanggal hari ini
        $today = Carbon::now()->format('Y-m-d');
    ...
    // function quiz listening practice test view
    public function quizListeningPracticeTest($levelId)
    {
        $user = Auth::user();
        ...
        return view('Features.english-zone.student.quiz.english-zone-quiz-listening-practice-test', compact('levelId'));
    }

    public function quizListeningPracticeTestForm($levelId)
    {
        // Ambil tanggal hari ini hanya dalam format 'Y-m-d'
        $today = Carbon::now()->format('Y-m-d');
    ...
    // function quiz listening practice test answer
    public function quizListeningPracticeTestAnswer(Request $request, $levelId, $passageId)
    {
        // Ambil tanggal hari ini
        $today = Carbon::now()->format('Y-m-d');
    ...
    // function quiz listening exam test view
    public function quizListeningExamTest($levelId)
    {
        $user = Auth::user();
        ...
        return view('Features.english-zone.student.quiz.english-zone-quiz-listening-exam-test', compact('levelId'));
    }

    // function quiz listening exam test form
    public function quizListeningExamTestForm($levelId)
    {
        // Ambil tanggal hari ini hanya dalam format 'Y-m-d'
        $today = Carbon::now()->format('Y-m-d');
        ...
        // Ambil semua passage Reading Exam Test
        $passageCollection = EnglishZonePassage::where('level_id', $levelId)->where('passage_type', 'Listening Exam Test')
        ->where('passage_status', 'Publish')->get();
        ...
        // Susun cache key khusus untuk urutan passage
        $passageCacheKey = "listening-passages-{$userId}-{$subscriptionId}-{$levelId}-{$passageIds}-{$lastUpdated}-{$limit}";
        ...
        // Ambil ulang soal-soal yang masih `Publish` dari DB dan tipe soal adalah `QUIZ`
        $publishedQuestionIds = EnglishZoneQuestions::whereHas('EnglishZonePassage', function ($query) {
            $query->where('passage_type', 'Listening Exam Test');
        })->where('level_id', $levelId)->where('tipe_soal', 'QUIZ')
        ->when($activePassage, fn($q) => $q->where('passage_id', $activePassage->id))
        ->where('status_bank_soal', 'Publish')->pluck('id')->implode(',');
        ...
        // susun cache key untuk urutan soal
        $cacheKey = "english-zone-quiz-listening-exam-test-{$userId}-{$subscriptionId}-{$levelId}-{$activePassageId}-{$lastUpdated}-{$publishedQuestionIds}";
    ...
    // function quiz listening exam test answer
    public function quizListeningExamTestAnswer(Request $request, $levelId, $passageId)
    {
        // Ambil tanggal hari ini
        $today = Carbon::now()->format('Y-m-d');
    ...
    // function quiz writing practice test view
    public function quizWritingPracticeTest($levelId)
    {
        $user = Auth::user();
    ...
    // function quiz writing practice test form
    public function quizWritingPracticeTestForm(Request $request, $levelId)
    {
        // Ambil tanggal hari ini hanya dalam format 'Y-m-d'
        $today = Carbon::now()->format('Y-m-d');
        ...
        $lastUpdated = EnglishZonePassage::whereIn('id', $passageIds)->max('updated_at');
        ...
        $pagedData = $collection->slice($offset, $perPage)->values();

        $passages = new LengthAwarePaginator(
            $pagedData,
            $collection->count(),
            $perPage,
            $page,
        ...
        if ($subscription) {
            // Ambil jawaban user sebelumnya untuk ditampilkan sebagai isian otomatis (per hari)
            $questionsAnswer = EnglishZoneAnswers::whereHas('EnglishZonePassage', function ($query) {
                    $query->where('passage_type', 'Writing Practice Test');
                })->where('student_id', $userId)->where('level_id', $levelId)->where('subscription_history_id', $subscription->id)
                ->whereDate('created_at', $today)->get()->mapWithKeys(function ($item) {
                    return [
    ...
    public function quizWritingPracticeTestAnswer(Request $request, $levelId, $passageId)
    {
        // Ambil tanggal hari ini
        $today = Carbon::now()->format('Y-m-d');
        ...
        $validator = Validator::make($request->all(), [
            ...
            'user_answer_text.required' => 'Harap isi jawaban.',
        ]);
    ...
    // function quiz speaking practice test view
    public function quizSpeakingPracticeTest($levelId)
    {
        $user = Auth::user();
        ...
        return view('Features.english-zone.student.quiz.english-zone-quiz-speaking-practice-test', compact('levelId'));
    }

    // function quiz speaking practice test form
    public function quizSpeakingPracticeTestForm(Request $request, $levelId)
    {
        // Ambil tanggal hari ini hanya dalam format 'Y-m-d'
        $today = Carbon::now()->format('Y-m-d');
        ...
        $passageIds = EnglishZonePassage::where('level_id', $levelId)->where('passage_type', 'Speaking Practice Test')
        ->where('passage_status', 'Publish')->pluck('id');
        ...
        // Buat key cache unik berdasarkan setiap hari, user, subscriptionId, levelId, passageId, questions, dan publishedQuestionIds
        $cacheKey = "english-zone-quiz-speaking-practice-test-{$today}-{$userId}-{$subscriptionId}-{$levelId}-{$passageIdString}-{$lastUpdated}-{$limit}";
        ...
        if ($subscription) {
            // Ambil jawaban user sebelumnya untuk ditampilkan sebagai isian otomatis (per hari)
            $questionsAnswer = EnglishZoneAnswers::whereHas('EnglishZonePassage', function ($query) {
                    $query->where('passage_type', 'Speaking Practice Test');
                })->where('student_id', $userId)->where('level_id', $levelId)
                ->when($subscription, fn($q) => $q->where('subscription_history_id', $subscription->id))
                ->whereDate('created_at', $today)->get()->mapWithKeys(function ($item) {
                    return [
                        $item->passage_id => $item->user_answer_audio
                    ];
                });
        } else {
            // Handle ketika user tidak punya subscription aktif
    ...
    // function quiz speaking practie test answer
    public function quizSpeakingPracticeTestAnswer(Request $request, $levelId, $passageId)
    {
        // Ambil tanggal hari ini
        $today = Carbon::now()->format('Y-m-d');
        ...
        $validator = Validator::make($request->all(), [
            'user_answer_audio' => 'required',
        ], [
            'user_answer_audio.required' => 'Jawaban tidak boleh kosong.',
        ]);
        ...
        if ($request->hasFile('user_answer_audio')) {
            $audioFile = $saveFileByHash($request->file('user_answer_audio'), 'english-zone-audio');
        }
    ...
    // function quiz speaking exam test view
    public function quizSpeakingExamTest($levelId)
    {
        $user = Auth::user();
        ...
        return view('Features.english-zone.student.quiz.english-zone-quiz-speaking-exam-test', compact('levelId'));
    }

    // function quiz speaking exam test form
    public function quizSpeakingExamTestForm(Request $request, $levelId)
    {
        // Ambil tanggal hari ini hanya dalam format 'Y-m-d'
        $today = Carbon::now()->format('Y-m-d');
        ...
        $passageIds = EnglishZonePassage::where('level_id', $levelId)->where('passage_type', 'Speaking Exam Test')
        ->where('passage_status', 'Publish')->pluck('id');
        ...
        // Buat key cache unik berdasarkan setiap hari, user, subscriptionId, levelId, passageId, questions, dan publishedQuestionIds
        $cacheKey = "english-zone-quiz-speaking-exam-test-{$userId}-{$subscriptionId}-{$levelId}-{$passageIdString}-{$lastUpdated}-{$limit}";
        ...
        if ($subscription) {
            // Ambil jawaban user sebelumnya untuk ditampilkan sebagai isian otomatis (per hari)
            $questionsAnswer = EnglishZoneAnswers::whereHas('EnglishZonePassage', function ($query) {
                    $query->where('passage_type', 'Speaking Exam Test');
                })->where('student_id', $userId)->where('level_id', $levelId)
                ->when($subscription, fn($q) => $q->where('subscription_history_id', $subscription->id))
    ...
    // function quiz speaking exam test answer
    public function quizSpeakingExamTestAnswer(Request $request, $levelId, $passageId)
    {
        // Ambil tanggal hari ini
        $today = Carbon::now()->format('Y-m-d');
