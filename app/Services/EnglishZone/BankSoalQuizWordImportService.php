...
class BankSoalQuizWordImportService
{
    public function bankSoalQuizWordImportService(Request $request, $level_id, $passage_id, $passage_type)
    {
        ...
        $validator = Validator::make($request->all(), [
            ...
            'bulkUpload-soal-quiz-english-zone' => 'required|file|mimes:docx|max:10000',
        ], [
            // Pesan error custom
            'bulkUpload-soal-quiz-english-zone.required' => 'File tidak boleh kosong.',
            'bulkUpload-soal-quiz-english-zone.max' => 'Ukuran file melebihi kapasitas yang ditentukan.',
        ]);
        ...
        $uploadedFile = $request->file('bulkUpload-soal-quiz-english-zone');
        ...
        if ($uploadedFile) {
            ...
            foreach ($tables as $index => $table) {
                ...
                $requiredFields = array_merge($presentOptions, ['ANSWER', 'EXPLANATION', 'LEVEL', 'DIFFICULTY', 'QUESTION_FORMAT']);
                    ...
                    foreach ($answerMap as $optionField => $label) {
                        if (!empty($dataSoal[$optionField])) {
                            $createBankSoal = EnglishZoneQuestions::create([
                                ...
                                'tipe_soal' => 'QUIZ',
                                'question_format' => trim(strip_tags($dataSoal['QUESTION_FORMAT'] ?? '')),
                            ]);
                        }
                    }
                }
