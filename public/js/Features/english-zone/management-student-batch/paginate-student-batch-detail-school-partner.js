function paginateStudentBatchDetail() {
    ...
    const schoolPartnerId = container.dataset.schoolPartnerId;
    ...
    if (!schoolPartnerId) return;

    fetchDataStudentBatchDetail(featureVariantId, levelId, batchId, batchScheduleGroups, batchScheduleId, studentIds, schoolPartnerId);

    function fetchDataStudentBatchDetail(page = 1) {
        $.ajax({
            url: `/english-zone/management-student-batch-detail/school-partner/${featureVariantId}/${levelId}/${batchId}/${batchScheduleGroups}/${batchScheduleId}/${studentIds}/${schoolPartnerId}/paginate`,
            method: 'GET',
            data: {
                page: page,
            },
                    ...
                    $.each(response.data, function (index, items) {
                        ...
                        <tr class="text-xs">
                            ...
                            <td class="td-table !text-black !text-center">
                                ...
                                data-hours="${scheduleHours}" data-student-batch-ids="${formattedIds}" data-school-partner-id="${schoolPartnerId ?? null}" data-transaction-source="${first.feature_subscription_history?.transactions?.transaction_source}">
                                    Re-Schedule
                                </button>
                            </td>
                            <td class="td-table !text-black !text-center">
...
$(document).off('click', '.btn-reschedule').on('click', '.btn-reschedule', function (e) {
    ...
    $('#reschedule-student-batch-form').data('school_partner_id', schoolPartnerId);
    ...
    $('#input-school-partner-id').val(schoolPartnerId);
...
$(document).ready(function () {
    ...
    const container = document.getElementById('container-management-student-batch-detail');
    const btnReschedule = $('.btn-reschedule');
    if (!container) return;

    const featureVariantId = container.dataset.featureVariantId;
    const levelId = container.dataset.levelId;
    const schoolId = container.dataset.schoolPartnerId;
    ...
    $('#day_of_week_id').on('change', function () {
        ...
        if (group_id && batch_id) {
            $.ajax({
                url: `/english-zone/management-student-batch-detail/school-partner/dropdown-hours/${batch_id}/${group_id}/${level_id}/${feature_variant_id}/${transactionSource}/${schoolPartnerId}`,
                type: 'GET',
                dataType: 'json',
                success: function (data) {
