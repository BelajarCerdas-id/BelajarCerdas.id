...
function quizListeningPracticeTest(page = 1, selectedIndex = 0) {
    const container = document.getElementById('container-quiz-listening-practice-test');
    if (!container) return;

    const levelId = container.dataset.levelId;
    if (!levelId) return;

    fetchquizListeningPracticeTest(levelId, page, selectedIndex);

    function fetchquizListeningPracticeTest(levelId, page, selectedIndex) {
        $.ajax({
            url: `/english-zone/${levelId}/quiz/listening-practice-test/form`,
            method: 'GET',
            data: {
                page: page,
            },
            success: function (response) {
                ...
                // jika tidak ada soal yang aktif pada passage, maka tampilkan pesan
                    ...
                    $('.pagination-container-question-listening-practice-test').html(response.links);
                ...
                // jika passage dan soal aktif, maka tampilkan data
                    ...
                    $.each(response.data, function (index, item) {
                        ...
                        let passageContent = item.audio_script;
                        ...
                        const generateOptions = (group) => {
                            ...
                            return shuffleOptions.map((item, index) => {
                                ...
                                if (questionsAnswer[selectedQuestionGroup[0].id] === selectedQuestionGroup[0].answer_key
                                    && questionsAnswer[selectedQuestionGroup[0].id] === item.options_key) {
                                    statusClass = 'bg-green-200 text-green-600 font-bold';

                                    // Jika jawab salah
                                } else if (questionsAnswer[selectedQuestionGroup[0].id] !== selectedQuestionGroup[0].answer_key
                                    && questionsAnswer[selectedQuestionGroup[0].id] === item.options_key) {
                                    statusClass = 'bg-red-200 text-red-600 font-bold';

                                    // Jika sudah jawab tapi bukan ini pilihannya, maka highlight jawaban benar
                                } else if (questionsAnswer[selectedQuestionGroup[0].id] && item.options_key === selectedQuestionGroup[0].answer_key) {
                                    statusClass = 'bg-green-200 text-green-600 font-bold';
                                }
                        ...
                        const isCorrect = questionsAnswer[selectedQuestionGroup[0].id] === selectedQuestionGroup[0].answer_key;
                        ...
                        const content = `
                            <div class="grid grid-cols-1 xl:grid-cols-4 bg-white shadow rounded-xl border">
                                ...
                                <div class="col-span-3">
                                    <div class="flex flex-col sm:flex-row sm:justify-between pt-6 px-6 gap-4 sm:gap-2">
                                        ...
                                        <div class="flex justify-center font-semibold">
                                            <span class="text-lg"> Part ${page} </span>
                                        </div>

                                        <!-- Listening + Level (mobile) -->
                                        <div class="flex flex-row items-center justify-between gap-4 text-xs sm:hidden">
                                            <span>Listening Practice Test</span>
                                            <span>${response.levelName}</span>
                                        </div>

                                        <!-- Listening (tengah, muncul sm ke atas) -->
                                    ...
                                    <div class="flex mx-4 relative z-[999999] pointer-events-auto"> 
                                        <audio controls controlsList="nodownload" id="audio-${item.id}">
                                            <source src="/english-zone-audio/${item.audio_file}" type="audio/mpeg">
                                            Browser kamu tidak mendukung audio tag.
                                        </audio>
                                    </div>

                                    <div class="toggleButton-passage pt-5 cursor-pointer block text-sm px-6">
                                        <div class="flex flex-col gap-4">
                                            <div class="flex items-center justify-between gap-2 border-y border-gray-300">
                                                <div class="flex items-center gap-2 w-full my-2">
                                                    Audioscript
                                                </div>
                                                <i class="fa-solid fa-chevron-up icon my-2"></i>
                                            </div>
                                        </div>

                                        <div class="content-accordion-passage overflow-y-auto max-h-[500px]">
                                            <div class="max-w-[1450px] text-sm mt-6">
                                                <div class="passage-text space-y-4">
                                                    ${passage}
                                                </div>
                                            </div>
                                        </div>
                                ...
                                <div class="col-span-1 p-6 space-y-4 bg-gray-50 border mt-8 xl:mt-0">
                                    ...
                                    <div id="container-questions-number" class="grid grid-cols-6 gap-1 text-center overflow-y-auto max-h-[600px] text-xs">
                                        ${nomorSoalHTML}
                                    </div>
                                </div>
                                ...
                                <div class="col-span-3 mt-8">
                                    <form id="bank-soal-quiz-practice-listening-test-question-form" data-level-id="${levelId}" data-passage-id="${activePassageId}">
                    ...
                    $(document).off('click', '.nomor-soal').on('click', '.nomor-soal', function () {
                        ...
                        quizListeningPracticeTest(page, index);
                    });
                } else {
...
$(document).ready(function () {
    quizListeningPracticeTest();
});

function bindPaginationLinks() {
    $('.pagination-container-question-listening-practice-test').off('click', 'a').on('click', 'a', function (event) {
        event.preventDefault(); // Cegah perilaku default link
        const page = new URL(this.href).searchParams.get('page'); // Dapatkan nomor halaman dari link
        quizListeningPracticeTest(page, currentQuestionIndex); // Ambil data yang difilter untuk halaman yang ditentukan
    });
}
...
// Submit form jawaban
$(document).on('submit', '#bank-soal-quiz-practice-listening-test-question-form', function (e) {
    e.preventDefault();
    ...
    $.ajax({
        url: `/english-zone-student/${levelId}/${passageId}/quiz/listening-practice-test/answers`,
        method: 'POST',
        headers: {
        ...
        success: function (response) {
            ...
            quizListeningPracticeTest(page, currentQuestionIndex);
