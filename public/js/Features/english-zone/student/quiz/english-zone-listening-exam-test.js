...
function quizListeningExamTest(page = 1, selectedIndex = 0) {
    const container = document.getElementById('container-quiz-listening-exam-test');
    if (!container) return;

    const levelId = container.dataset.levelId;
    if (!levelId) return;

    fetchquizListeningExamTest(levelId, page, selectedIndex);

    function fetchquizListeningExamTest(levelId, page, selectedIndex) {
        $.ajax({
            url: `/english-zone/${levelId}/quiz/listening-exam-test/form`,
            method: 'GET',
            data: {
                page: page,
            },
            success: function (response) {
                ...
                // jika tidak ada soal yang aktif pada passage, maka tampilkan pesan
                    ...
                    $('.pagination-container-question-listening-exam-test').html(response.links);
                ...
                // jika passage dan soal aktif, maka tampilkan data
                        ...
                        const content = `
                            <div class="grid grid-cols-1 xl:grid-cols-4 bg-white shadow rounded-xl border">
                                ...
                                <div class="col-span-3 mt-8">
                                    <form id="bank-soal-quiz-exam-listening-test-question-form" data-level-id="${levelId}" data-passage-id="${activePassageId}">
                    ...
                    $(document).off('click', '.nomor-soal').on('click', '.nomor-soal', function () {
                        ...
                        quizListeningExamTest(page, index);
                    });
                } else {
...
$(document).ready(function () {
    quizListeningExamTest();
});

function bindPaginationLinks() {
    $('.pagination-container-question-listening-exam-test').off('click', 'a').on('click', 'a', function (event) {
        event.preventDefault(); // Cegah perilaku default link
        const page = new URL(this.href).searchParams.get('page'); // Dapatkan nomor halaman dari link
        quizListeningExamTest(page, currentQuestionIndex); // Ambil data yang difilter untuk halaman yang ditentukan
    });
}
...
// Submit form jawaban
$(document).on('submit', '#bank-soal-quiz-exam-listening-test-question-form', function (e) {
    e.preventDefault();
    ...
    $.ajax({
        url: `/english-zone-student/${levelId}/${passageId}/quiz/listening-exam-test/answers`,
        method: 'POST',
        headers: {
        ...
        success: function (response) {
            ...
            quizListeningExamTest(page, currentQuestionIndex);
