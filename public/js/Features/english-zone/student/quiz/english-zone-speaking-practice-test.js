...
function quizSpeakingPracticeTest(page = 1, selectedIndex = 0) {
    const container = document.getElementById('container-quiz-speaking-practice-test');
    if (!container) return;

    const levelId = container.dataset.levelId;
    if (!levelId) return;

    fetchquizSpeakingPracticeTest(levelId, page, selectedIndex);

    function fetchquizSpeakingPracticeTest(levelId, page, selectedIndex) {
        $.ajax({
            url: `/english-zone/${levelId}/quiz/speaking-practice-test/form`,
            method: 'GET',
            data: {
                page: page,
            },
            success: function (response) {
                ...
                // jika passage dan soal aktif, maka tampilkan data
                        ...
                        const content = `
                            <div class="grid grid-cols-1 xl:grid-cols-4 bg-white shadow rounded-xl border">
                                ...
                                <div class="col-span-2">
                                    <div class="flex flex-col sm:flex-row sm:justify-between pt-6 px-6 gap-4 sm:gap-2">
                                        ...
                                        <div class="flex flex-row items-center justify-between gap-4 text-xs sm:hidden">
                                            <span>Speaking Practice Test</span>
                                ...
                                <div class="col-span-2 border px-4 mt-8 xl:mt-0">
                                    <form id="speaking-practice-test-form" data-level-id="${levelId}" data-passage-id="${activePassageId}">
                    ...
                    $('.pagination-container-question-speaking-practice-test').html(response.links);
                    bindPaginationLinks();
                } else {
...
$(document).ready(function () {
    quizSpeakingPracticeTest();
});

function bindPaginationLinks() {
    $('.pagination-container-question-speaking-practice-test').off('click', 'a').on('click', 'a', function (event) {
        event.preventDefault(); // Cegah perilaku default link
        const page = new URL(this.href).searchParams.get('page'); // Dapatkan nomor halaman dari link
        quizSpeakingPracticeTest(page, currentQuestionIndex); // Ambil data yang difilter untuk halaman yang ditentukan
    });
}

function checkSubscription() {
    const levelId = $('#speaking-practice-test-form').data('level-id');
    $.ajax({
        url: `/english-zone/${levelId}/quiz/speaking-practice-test/form`,
        method: 'GET',
        success: function (response) {
...
// Submit form jawaban
$(document).on('submit', '#speaking-practice-test-form', function (e) {
    e.preventDefault();
    ...
    $.ajax({
        url: `/english-zone-student/${levelId}/${passageId}/quiz/speaking-practice-test/answers`,
        method: 'POST',
        headers: {
