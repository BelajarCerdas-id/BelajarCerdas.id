...
function quizReadingPracticeTest(page = 1, selectedIndex = 0) {
    const container = document.getElementById('container-quiz-reading-practice-test');
    if (!container) return;

    const levelId = container.dataset.levelId;
    if (!levelId) return;

    fetchquizReadingPracticeTest(levelId, page, selectedIndex);

    function fetchquizReadingPracticeTest(levelId, page, selectedIndex) {
        $.ajax({
            url: `/english-zone/${levelId}/quiz/reading-practice-test/form`,
            method: 'GET',
            data: {
                page: page,
            },
            success: function (response) {
                ...
                // jika tidak ada soal yang aktif pada passage, maka tampilkan pesan
                    ...
                    $('.pagination-container-question-reading-practice-test').html(response.links);
                ...
                // jika passage dan soal aktif, maka tampilkan data
                        ...
                        const content = `
                            <div class="grid grid-cols-1 xl:grid-cols-4 bg-white shadow rounded-xl border">
                                ...
                                <div class="col-span-3">
                                    <div class="flex flex-col sm:flex-row sm:justify-between pt-6 px-6 gap-4 sm:gap-2">
                                        ...
                                        <div class="flex flex-row items-center justify-between gap-4 text-xs sm:hidden">
                                            <span>Reading Practice Test</span>
                                ...
                                <div class="col-span-3 mt-8">
                                    <form id="bank-soal-quiz-practice-reading-test-question-form" data-level-id="${levelId}" data-passage-id="${activePassageId}">
                    ...
                    $(document).off('click', '.nomor-soal').on('click', '.nomor-soal', function () {
                        ...
                        quizReadingPracticeTest(page, index);
                    });
                } else {
...
$(document).ready(function () {
    quizReadingPracticeTest();
});

function bindPaginationLinks() {
    $('.pagination-container-question-reading-practice-test').off('click', 'a').on('click', 'a', function (event) {
        event.preventDefault(); // Cegah perilaku default link
        const page = new URL(this.href).searchParams.get('page'); // Dapatkan nomor halaman dari link
        quizReadingPracticeTest(page, currentQuestionIndex); // Ambil data yang difilter untuk halaman yang ditentukan
    });
}
...
// Submit form jawaban
$(document).on('submit', '#bank-soal-quiz-practice-reading-test-question-form', function (e) {
    e.preventDefault();
    ...
    $.ajax({
        url: `/english-zone-student/${levelId}/${passageId}/quiz/reading-practice-test/answers`,
        method: 'POST',
        headers: {
        ...
        success: function (response) {
            ...
            quizReadingPracticeTest(page, currentQuestionIndex);
