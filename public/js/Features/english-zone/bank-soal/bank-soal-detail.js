function paginateBankSoalDetailEZ(search_question = '') {
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                if (response.data.length > 0) {
                    ...
                    // bindPaginationLinks(subBab, levelId);
    ...
    // function bindPaginationLinks(subBab, levelId) {
    //     $('.pagination-container-bank-soal-detail').off('click', 'a').on('click', 'a', function(event) {
    //         event.preventDefault(); // Cegah perilaku default link
    //         const page = new URL(this.href).searchParams.get('page'); // Dapatkan nomor halaman dari link
    //         fetchFilteredDataBankSoalDetailEZ(subBab, levelId, page); // Ambil data yang difilter untuk halaman yang ditentukan
    //     });
    // }
}
...
$('#search_question').on('input', function() {
    ...
    paginateBankSoalDetailEZ(search_question); // Call the function to fetch data based on search_school
});
