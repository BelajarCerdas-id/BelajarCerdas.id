function paginateMentorStudentBatchSchoolPartner(page = 1) {
    $.ajax({
        url: '/english-zone-mentor/student-batch/school-partner/paginate',
        method: 'GET',
        data: {
            page: page,
        },
        success: function (response) {
            ...
            if (response.data.length > 0) {
                ...
                $('.pagination-container-management-student-batch-school-partner').html(response.links);
                bindPaginationLinks(); // Bind click event ke link pagination yang baru
                $('#empty-message-management-student-batch-school-partner').hide(); // sembunyikan pesan kosong
                $('.thead-table-management-student-batch-school-partner').show(); // Tampilkan tabel thead
            } else {
                $('#tbody-table-management-student-batch-school-partner').empty(); // Clear existing rows
                $('#empty-message-management-student-batch-school-partner').show(); // Tampilkan pesan kosong
                $('.thead-table-management-student-batch-school-partner').hide(); // sembunyikan tabel thead
            }
        },
        error: function (xhr) {
...
$(document).ready(function () {
    paginateMentorStudentBatchSchoolPartner();
});
...
function bindPaginationLinks() {
    $('.pagination-container-management-student-batch-school-partner').off('click', 'a').on('click', 'a', function (event) {
        ...
        paginateMentorStudentBatchSchoolPartner(page); // Ambil data yang difilter untuk halaman yang ditentukan
    });
}
